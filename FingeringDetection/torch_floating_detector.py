...
def process_raw_video_with_mediapipe(video_path: str, video_name: str, 
    ...
    with mp_hands.Hands(
        ...
        # FPS 샘플링 계산
        fps_ratio = int(original_fps / target_fps) if target_fps < original_fps else 1
        estimated_frames = total_frames // fps_ratio if fps_ratio > 1 else total_frames
        
        print(f"   📊 비디오 정보: {width}x{height}, {original_fps:.1f}FPS → {target_fps}FPS 샘플링")
        print(f"   📈 원본 프레임: {total_frames:,}개 → 처리 예상: {estimated_frames:,}개")
        print(f"   🎯 FPS 비율: 1/{fps_ratio} (매 {fps_ratio}번째 프레임 처리)")
        
        # 프레임 제한 적용 (None이면 전체 처리)
        process_frames = total_frames
        if frame_limit and total_frames > frame_limit:
            process_frames = frame_limit
            print(f"   ⏱️  프레임 제한 적용: {frame_limit:,}프레임으로 제한")
        else:
            print(f"   🎬 전체 프레임 처리: {total_frames:,}프레임")
        
        handlist = []
        video_frame_count = 0  # 실제 비디오 프레임 카운터
        processed_frame_count = 0  # 처리된 프레임 카운터
        
        if HAS_STQDM:
            progress_bar = stqdm(range(process_frames), desc=f"🎬 {video_name} MediaPipe 처리 ({target_fps}FPS)")
        else:
            progress_bar = range(process_frames)
            print(f"   🔄 {process_frames:,}프레임 처리 중... (진행률 표시: pip install stqdm)")
        
        for _ in progress_bar:
            if video_frame_count >= process_frames:
                break
            ...
            # FPS 샘플링 적용 (매 fps_ratio 프레임마다 처리, 빈 프레임 추가 안함)
            if fps_ratio > 1 and video_frame_count % fps_ratio != 0:
                video_frame_count += 1
                continue  # 프레임 스킵만, 빈 프레임 추가 안함
                ...
                for j in range(len(results.multi_handedness)):
                    ...
                    try:
                        ...
                        if hasattr(handedness, 'classification') and len(handedness.classification) > 0:
                            ...
                            if processed_frame_count < 10:  # 첫 10프레임만 디버깅 출력
                                print(f"   🔍 감지된 손 타입: {hand_type} (처리 프레임 {processed_frame_count})")
                        else:
                            hand_type = "Left" if j == 0 else "Right"  # 기본값
                            if processed_frame_count < 10:  # 첫 10프레임만 디버깅 출력
                                print(f"   ⚠️ handedness 정보 없음, 기본값 사용: {hand_type} (처리 프레임 {processed_frame_count})")
                    except (AttributeError, IndexError) as e:
                        hand_type = "Left" if j == 0 else "Right"  # 기본값
                        if processed_frame_count < 10:  # 첫 10프레임만 디버깅 출력
                            print(f"   ⚠️ handedness 추출 실패: {e}, 기본값 사용: {hand_type} (처리 프레임 {processed_frame_count})")
                    
                    hand_obj = handclass(
                        handtype=hand_type,
                        handlandmark=landmarks.landmark,
                        handframe=processed_frame_count  # 처리된 프레임 번호 사용
                    )
        ...
        print(f"   🎬 FPS 샘플링: {original_fps:.1f} → {target_fps} (1/{fps_ratio} 프레임)")
        print(f"   📈 원본 프레임: {total_frames:,}개 → 처리됨: {len(handlist):,}개")
...
class TorchFloatingDetector:
    ...
    # 기본 설정
    QUICK_TEST = False  # 전체 프레임 처리
    FRAME_LIMIT = None  # 프레임 제한 없음
    TARGET_FPS = 20  # 20fps로 샘플링
    ...
    def _initialize_video_list(self):
        ...
        # MAX_VIDEOS 제한 적용 (None이면 전체 처리)
        if self.MAX_VIDEOS is not None and len(self.target_videos) > self.MAX_VIDEOS:
            ...
            print(f"📝 전체 {len(self.target_videos)}개 비디오 처리")
            # 최신 파일 순으로 정렬
            self.target_videos = sorted(self.target_videos, reverse=True)
    ...
    def load_data(self, data_info: Dict[str, Any]) -> Tuple[List, List, float]:
        ...
        if data_info.get('is_raw_video', False):
            ...
            handlist, ratio = process_raw_video_with_mediapipe(
                ...
                target_fps=self.target_fps, # 새로운 매개변수 추가
    ...
    def create_result_video(self, data_info: Dict[str, Any], floating_results: List, handlist: List) -> str:
        ...
        try:
            ...
            # FPS 샘플링 계산 (분석과 동일하게)
            fps_ratio = int(original_fps / self.target_fps) if self.target_fps < original_fps else 1
            output_fps = self.target_fps
            
            # 정확한 프레임 계산: 실제 handlist 크기 기준
            actual_processed_frames = len(handlist)
            max_possible_frames = total_frames // fps_ratio if fps_ratio > 1 else total_frames
            
            # Quick test인 경우 프레임 제한
            if self.quick_test and self.frame_limit:
                total_frames = min(total_frames, self.frame_limit)
                max_possible_frames = min(max_possible_frames, self.frame_limit // fps_ratio)
            
            # 실제 출력 프레임 수는 handlist 크기로 제한
            estimated_output_frames = min(actual_processed_frames, max_possible_frames)
            
            # 비디오 라이터 설정 (20fps로 출력)
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            out = cv2.VideoWriter(output_path, fourcc, output_fps, (width, height))
            
            print(f"   ? 해상도: {width}x{height}")
            print(f"   ? FPS: {original_fps} → {output_fps} (1/{fps_ratio} 프레임)")
            print(f"   📊 Floating 결과: {len(floating_results)}개")
            
            # floating 결과를 프레임별로 정리 (handlist 기반 인덱스 사용)
            floating_by_frame = {}
            for result in floating_results:
                # floating 결과 파싱: [frame, handtype, metric_value, 'floating'/'notfloating']
                frame_num = result[0]  # handlist의 frame 인덱스
            ...
            # 데이터 일치성 검증
            max_handframe = max([hand.handframe for hands in handlist if hands for hand in hands]) if any(handlist) else -1
            max_floating_frame = max([r[0] for r in floating_results]) if floating_results else -1
            
            print(f"🔍 정확한 프레임 계산:")
            print(f"   실제 처리된 프레임: {actual_processed_frames}")
            print(f"   이론적 최대 프레임: {max_possible_frames}")
            print(f"   출력 예상 프레임: {estimated_output_frames}")
            print(f"   handlist 프레임 범위: 0 ~ {max_handframe}")
            print(f"   floating 프레임 범위: 0 ~ {max_floating_frame}")
            print(f"   floating_by_frame 크기: {len(floating_by_frame)}")
            
            # handlist를 프레임별로 정리 (성능 최적화)
            handlist_by_frame = {}
            for hands in handlist:
            ...
            print(f"⚡ 성능 최적화:")
            print(f"   MediaPipe 재실행 제거 → 기존 handlist 데이터 활용")
            print(f"   handlist_by_frame 준비 완료: {len(handlist_by_frame)}개 프레임")
            print(f"   무한 루프 방지 → 명확한 종료 조건 설정")
            
            video_frame_count = 0  # 실제 비디오 프레임 카운터
            processed_frame_count = 0  # 처리된 프레임 카운터 (handlist와 매칭)
            
            # 무한 루프 방지: 명확한 종료 조건
            while (processed_frame_count < estimated_output_frames and 
                ...
                # Quick test 프레임 제한 확인
                if self.quick_test and self.frame_limit and video_frame_count >= self.frame_limit:
                    break
                
                # FPS 샘플링 적용 (20fps에 맞게 프레임 스킵)
                if fps_ratio > 1 and video_frame_count % fps_ratio != 0:
                    video_frame_count += 1
                    continue
                ...
                if processed_frame_count % 100 == 0:
                    progress = (processed_frame_count / estimated_output_frames) * 100 if estimated_output_frames > 0 else 0
                    print(f"   ? 진행: {processed_frame_count}/{estimated_output_frames} ({progress:.1f}%)")
                
                # 현재 프레임의 floating 상태 가져오기 (processed_frame_count를 handlist 인덱스로 사용)
                current_floating = floating_by_frame.get(processed_frame_count, {})
                current_depth = {
                    'Left_depth': current_floating.get('Left_depth', 0.0),
                    'Right_depth': current_floating.get('Right_depth', 0.0)
                }
                
                # 기존 handlist에서 손 데이터 가져오기 (MediaPipe 재실행 대신)
                current_hands = handlist_by_frame.get(processed_frame_count, [])
                
                # 직접 시각화 적용 (MediaPipe 객체 변환 생략)
                annotated_frame = self.draw_hands_directly(
                    frame, 
                    current_hands,
                    current_floating,
                    current_depth,
            ...
            print(f"   📊 출력 프레임: {processed_frame_count}개 ({output_fps}fps)")
            print(f"   🎬 비디오 길이: {processed_frame_count/output_fps:.1f}초")
            print(f"   ⚡ 성능: MediaPipe 재실행 제거 + 직접 시각화")
            print(f"   🔒 종료 조건: processed_frame_count={processed_frame_count}, estimated={estimated_output_frames}")
            monitor.finish_step(f"비디오 생성 완료: {processed_frame_count}프레임 ({output_fps}fps)")
    ...
    def draw_hands_directly(self, image, hands, floating_status, depth_info, threshold_value=0.9):
        """handlist 데이터를 직접 사용한 손 시각화 (MediaPipe 변환 생략)"""
        if not hands:
            return image
        ...
        for hand in hands:
            ...
            # 손 랜드마크 직접 그리기 ([-1,1] 좌표 직접 사용)
            landmarks = hand.handlandmark
            landmark_points = []
            
            for landmark in landmarks:
                # [-1,1] → [0,width/height] 변환
                ...
                if 0 <= x < width and 0 <= y < height:
                    ...
                    cv2.circle(annotated_image, (x, y), 4, (255, 255, 255), 1)  # 흰색 테두리
                else:
                    landmark_points.append(None)  # 화면 밖 점
            
            # 손가락 연결선 그리기 (MediaPipe HAND_CONNECTIONS 대신 직접 구현)
            hand_connections = [
                (0, 1), (1, 2), (2, 3), (3, 4),  # 엄지
                (0, 5), (5, 6), (6, 7), (7, 8),  # 검지
                ...
                (5, 9), (9, 13), (13, 17)  # 손가락 간 연결
            ]
            
            for start_idx, end_idx in hand_connections:
                if (start_idx < len(landmark_points) and end_idx < len(landmark_points) and
                    landmark_points[start_idx] is not None and landmark_points[end_idx] is not None):
                    cv2.line(annotated_image, landmark_points[start_idx], landmark_points[end_idx], 
                            connection_color, 2)
            
            # 텍스트 정보 표시
            if landmark_points and landmark_points[0] is not None:  # 손목 위치 기준
                text_x, text_y = landmark_points[0]
                text_y = max(30, text_y - 30)  # 위쪽에 표시
                
                # 손 종류 표시
                cv2.putText(annotated_image, f"{hand_type} Hand", 
                           (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                
                # 상태 표시
                cv2.putText(annotated_image, f"Status: {status_text}", 
                           (text_x, text_y + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, status_color, 2)
                
                # 깊이 값 표시
                cv2.putText(annotated_image, f"Depth: {depth_value:.3f}", 
                           (text_x, text_y + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    ...
    def run_detection(self) -> Dict[str, Any]:
        """전체 detection 실행"""
        if self.quick_test and self.frame_limit:
            mode_text = f"({self.frame_limit}프레임 제한)"
        else:
            mode_text = f"(전체 프레임, {self.target_fps}FPS 샘플링)"
...
def process_multiple_videos():
    ...
    print(f"   💾 깊이 데이터 저장")
    print(f"   📹 결과 비디오 생성")
    print(f"   🎬 전체 비디오 처리 ({detector.target_fps}FPS 샘플링)")
    print(f"   📊 총 {len(detector.target_videos)}개 비디오 처리")
    print("")
...
if __name__ == "__main__":
    ...
    # 통합 모드 자동 실행
    print(f"📊 현재 설정: 전체 비디오 처리, 20FPS 샘플링, 임계값 {TorchFloatingDetector.FIXED_THRESHOLD}")
